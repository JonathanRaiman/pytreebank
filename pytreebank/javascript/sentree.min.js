...
function drawTree(line, data, highlightClass, err) {
  ...
  function callback() {
    ...
    if ($scope.oldscore == $scope.newscore) {
      send($scope, "jreinf", loggerURL);
    } else {
      /** @type {number} */
      options.rating = $scope.newscore;
      instances.push($scope);
      send($scope, "json", loggerURL);
      init(line, options);
    }
  }
  /**
...
/** @type {string} */
var baseURL2 = "http://nlp.stanford.edu:8080/sentiment/";
/** @type {string} */
var loggerURL = baseURL2 + "logger.html";
/** @type {Date} */
var date = new Date;
